...
void move(struct Position *pos, char instruction) {
  switch (instruction) {
  case LEFT:
    pos->x--;
    break;
...
int main(int argc, char **argv) {
  ...
  for (char instruction = first_argument[instruction_iterator];
    ...
    move(&santa_p1, instruction);
    move(current_santa_p2, instruction);
  ...
  printf("Part One: %zu; Part Two: %zu", history_p1.size, history_p2.size);
