...
int apply_to_grid(uint8_t *grid, size_t width, size_t height,
                  const struct Instruction *instruction,
                  int (*apply_to_pixel)(uint8_t *, const enum Action)) {

  assert(instruction->dest.width < width &&
         "Instruction width bigger than grid width");
  assert(instruction->dest.height < height &&
         "Instruction height bigger than grid height");

  for (size_t y = 0; y <= instruction->dest.height; y++) {
    for (size_t x = 0; x <= instruction->dest.width; x++) {
      int rc = apply_to_pixel(
          &grid[(instruction->dest.anchor.y + y) * LIGHT_GRID_HEIGHT +
                (instruction->dest.anchor.x + x)],
          instruction->action);
      if (rc != 0) {
        return rc;
      }
...
int main(int argc, char **argv) {
  ...
  uint8_t *light_grid_p1 =
      calloc(LIGHT_GRID_HEIGHT * LIGHT_GRID_WIDTH, sizeof(uint8_t));
  uint8_t *light_grid_p2 =
      calloc(LIGHT_GRID_HEIGHT * LIGHT_GRID_WIDTH, sizeof(uint8_t));
  ...
  for (char *line = strtok(argv[1], "\n"); line != NULL;
    ...
    struct Instruction instruction = {.action = action_from_str(action_str),
                                      .dest = destination};
    int rc = apply_to_grid(light_grid_p1, LIGHT_GRID_WIDTH, LIGHT_GRID_WIDTH,
                           &instruction, apply_to_pixel_p1);
    if (rc != 0) {
      printf("Failed to apply instruction (%d)\n", rc);
      exit(EXIT_FAILURE);
    };

    rc = apply_to_grid(light_grid_p2, LIGHT_GRID_WIDTH, LIGHT_GRID_WIDTH,
                       &instruction, apply_to_pixel_p2);
    if (rc != 0) {
      printf("Failed to apply instruction (%d)\n", rc);
      exit(EXIT_FAILURE);
    };
